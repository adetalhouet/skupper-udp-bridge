...
Subscriber.prototype.on_message = function (context) {
    ...
    console.log('Got AMQP message %s, sending over UDP to %s:%s (reply address is %s)', data, this.udp_address.host, this.udp_address.port, reply_to)
...
Relay.prototype.on_message = function (data, rinfo) {
    ...
    this.mappings.get_reply_address(rinfo).then (function (reply_address) {
        console.log('Got UDP message %s, sending over AMQP to %s (reply path is %s -> %s:%s)', data, self.sender.target.address, reply_address, rinfo.address, rinfo.port);
